...
| `ethereum.rpc_timeout` | `60s` | Deadline per provider RPC attempt (headers, receipts, logs, subscribe); exceeded → retried within the 5-minute budget, then the call fails and ingestion exits |
...
| `rpc.query_timeout` | `60s` | Must be > 0. Bounds every warehouse read: an `eth_getLogs` query, the `eth_blockNumber` head read and the `/health` read (which returns `503` on expiry) |
...
`-stage` selects one stage instead of `all`: `prepare` (drop the secondary indexes), `logs` (COPY each `logs/part=NNN/` into its partition, sorted by `(block_number, log_index)`), `blocks` (COPY `blocks/*.parquet` into `eth_blocks`, trimmed to the manifest's block interval), `finish` (verify the database, the Parquet footers and the local files against `manifest.json` at the export root — required, generated from BigQuery counts and GCS checksums, see [docs/operations.md](docs/operations.md) §1.2 — then recreate the indexes, `ANALYZE`, publish the manifest's coverage; refuses and leaves the cursor unset on any mismatch or without a manifest). Every stage is idempotent: `logs` and `blocks` skip a unit only when the database already holds the manifest's row count for it *and* `backfill_units` records the same manifest fingerprint; any other state is cleared and reloaded, so re-running after a failure — or with a corrected export and its new manifest — resumes where it stopped and reloads exactly what changed.

**Do not load the rehearsal export** (`gs://eth-logs/eth-log-warehouse/v0-rehearsal`) into a warehouse you intend to query. It has the same layout but only the logs of one day (2026-08-20) against a *complete* `blocks/` export; `finish` publishes the manifest's interval as verified coverage, and a manifest describing that export would let every in-scope query outside that day get a false `[]` instead of a scope error — exactly what the coverage contract forbids. `finish` therefore requires a `manifest.json` generated from the export's source (the rehearsal prefix has none and is refused); only `v1` with its manifest (or a later full re-export with a freshly generated one, per [docs/probe_2026-08.md](docs/probe_2026-08.md)) is a valid `backfill` input. For a quick local run start with an empty warehouse: tail ingestion begins at the chain head and `coverage_start` keeps the served interval honest.
