...
**Manual rewind** — `ff-eth-logs rewind -to N` is also the tool for re-ingesting a range that is suspected stale for any other reason (a provider incident, a deliberate re-check). Stop the service, rewind, start; it resumes at `N+1`. `-to 0` is valid on a full-history warehouse (keeps genesis, re-ingests from block 1); the flag must be present — its absence, not its value, is the error.
