...
**Amendment 2026-08-30 — CryptoPunks internal `Transfer`.** The extract now also keeps every `Transfer`-signature log emitted by the CryptoPunks contract (3 topics; `eventset.Keep` does the same for the tail). The v1 export and any warehouse loaded from it lack those rows for the history below the tail switch-over, and the indexer refuses to route to such a warehouse — its startup probe asks block 3,919,706 for that log (ff-indexer-v2 #144). Bringing an existing warehouse up to date is a re-export with the amended `WHERE` clause (≈ $15–21, 2 minutes) and a full reload (`backfill` through `prepare`, `logs`, `blocks` and `finish` with its index rebuilds — **4 h 03 m measured** for the full history, see the durations table in §1; a partial reload of only the affected partitions still needs the `blocks` and `finish` stages); during the reload the API answers `warehouse is under maintenance` and the indexer falls through to the vendor, by design.
