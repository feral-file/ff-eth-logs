...
- **Readers are excluded during a backfill** — a backfill holds the maintenance lock exclusively for its whole run and every API read snapshot takes it shared, so a reader (including an API-only replica that holds no writer lock) is refused with a scope error while partitions are being reloaded under a still-published coverage, instead of being served a mix of old and new partitions.
