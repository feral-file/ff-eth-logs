...
- **Cursor never ahead of data** — `ingest_cursor.block_number` is the warehouse head and the bound every served range is checked against. It moves only inside the transaction that writes the blocks and logs it accounts for (`logstore.WriteRange`); the backfill sets it last, after every partition and `eth_blocks` are loaded. A crash can leave the cursor behind the data (the next batch re-fetches and overwrites), never ahead of it.
...
- **Deep reorgs are reported, never replayed** — a replaced height at or below the last written block logs `ethereum reorg deeper than confirmation lag: a written block was replaced` at error level (reaches Sentry) and the data stays as written until an operator runs `ff-eth-logs rewind`. There is no automatic rewrite, by design: an automatic rewind could loop on a flapping provider and the operator must see it.
...
- **Partitions per 1,000,000 blocks** — 40 created up front (to block 40 M, ~2031); ingestion creates the next one on demand. Partition boundaries match the export directories so the backfill loads one directory per partition.
...
- **Automatic reorg repair** — deep reorgs are an operator procedure.
