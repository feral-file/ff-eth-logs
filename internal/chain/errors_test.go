...
func TestIsTooManyResultsError(t *testing.T) {
	...
	cases := []struct {
		...
		{"uppercase phrasing is matched", errors.New("Query Returned More Than 10000 Results"), true},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tc.want, chain.IsTooManyResultsError(tc.err))
		})
	}
}
...
func TestIsBlockRangeCapError(t *testing.T) {
	...
	cases := []struct {
		...
		{"block range too wide", errors.New("block range is too wide"), true},
		{"range too large", errors.New("range too large"), true},
		{"range is too large", errors.New("the range is too large"), true},
		...
		{"invalid params mentioning a limit", &rpcError{code: -32602, msg: "over the limit"}, true},
		...
		{"limited-to without block range wording", errors.New("this key is limited to 10 requests per second"), false},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tc.want, chain.IsBlockRangeCapError(tc.err))
		})
	}
}

func TestSingleBlockOverflowError(t *testing.T) {
	t.Parallel()

	cause := errors.New("query returned more than 10000 results")
	err := error(&chain.SingleBlockOverflowError{Block: 42, Err: cause})
